...
public class DailyReport {
    ...
    public Hashtable getTotals() {
        ...
        end.setTimeInMillis(date.getTime() + 24 * 60 * 60 * 1000);

        try {
            StoreDB db = new StoreDB();
            ResultSet rs = db.query("select sum(orders_total) as sales_total, sum(orders_tax) as tax_total from orders where orders_status = false and orders.date_purchased > '" +
    ...
    public Hashtable getPaymentTotals() {
        ...
        try {
            ...
            ResultSet rs = db.query("select sum(orders_total) as sales_total,  payment_method from orders where orders_status = false and orders.date_purchased > '" +
                                    (new Timestamp(start.getTimeInMillis())) +
                                    "' and orders.date_purchased < '" +
                                    (new Timestamp(end.getTimeInMillis())) +
