...
public class SalesPanel extends JPanel implements ListSelectionListener {
    ...
    void showOrderDetails(int orderId) {
        jPanel2.removeAll();
        jPanel3.removeAll();
        OrdersProductsTableModel model2 = (OrdersProductsTableModel) jTable2.getModel();
    ...
    JPanel createPaymentPanel(Payment payment) {
        ...
        if (payType.equals("Cash")) {
            ...
            if (((CreditPayment) payment).getExpireMonth() != null) {
                ...
                rightPanel.add(new JLabel(((CreditPayment) payment).getExpireMonth()
                        + "/"
                        + ((CreditPayment) payment).getExpireYear()));
            }
